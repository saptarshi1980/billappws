...
public class TariffUploadController {
    ...
    @RequestMapping(value = "/tariffshow.htm", method = RequestMethod.POST)
	public String displayForm(@RequestParam("title") String title,@RequestParam("description") String description,HttpServletRequest request) {
    	...
    	return "tariff_upload_form";
	}
    

	@RequestMapping(value = "/uploadMultipleTariffFile.htm", method = RequestMethod.POST)
	public String savePdf(@ModelAttribute("uploadForm") FileUploadForm uploadForm,
        ...
        int status=tariffdao.saveTariff(title,description,id);
        ...
        if(null != files && files.size() > 0) {
            for (MultipartFile multipartFile : files) {
                ...
                int statusfile=tariffdao.saveTariffFile(id,fileName);
